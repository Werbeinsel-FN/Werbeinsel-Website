<?php
/**
 * Partial: Ambient Advertising – hero (naslov + slider) + intro tekst (FORCIRAN)
 * Vidi promenljive iz parent template-a: $slug, $svc, $page_title, $page_id
 */

/* 1) Poseban CSS samo za ovu stranicu */
$css_path = get_stylesheet_directory() . '/css/service-ambient-advertising.css';
$css_url  = get_stylesheet_directory_uri() . '/css/service-ambient-advertising.css';
...
if (function_exists('wi_get_ambient_meta')) {
  $meta = wi_get_ambient_meta($page_id);
...
$carousel_id = ($slug ?: 'ambient-advertising') . '-hero';

/* 4) NASLOV – uvek forsiran */
$intro_title = "AMBIENT\nADVERTISING";

/* 5) INTRO TEKST – uvek forsiran (ignorišemo meta['intro_text']) */
$intro_text = 'Ambient Advertising integriert Ihre Marke unauffällig, aber wirkungsvoll in den Alltag: an Orten, Objekten und Situationen, die Menschen tatsächlich berühren. Mit smarten Platzierungen und überraschenden Touchpoints entsteht Aufmerksamkeit ohne Medienbruch. WERBEINSEL konzipiert, produziert und orchestriert Ambient-Kampagnen rechtssicher, skalierbar und messbar – genau dort, wo Botschaften haften bleiben.';
?>
...
<section id="<?php echo esc_attr($carousel_id); ?>" class="svc-hero-carousel">
  <div class="svc-hero-carousel__wrap">
    ...
    <!-- viewport spušten ispod (da ne preklapa intro) -->
    <div class="svc-hero-carousel__viewport" style="z-index:1; pointer-events:none;">
      <?php foreach ($slides as $i => $url): ?>
        <div
          ...
          role="img"
          aria-label="Ambient Advertising Slide <?php echo (int)$i+1; ?>">
        </div>
      <?php endforeach; ?>
    </div>

    <!-- Navigacija i tačkice ostaju klikalni iznad -->
