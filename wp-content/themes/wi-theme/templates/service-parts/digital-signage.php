<?php
/**
 * Partial: Digital Signage – hero (naslov + slider) + intro (editabilno)
 * Vidi: $slug, $svc, $page_title, $page_id
 */

/* 1) Page-specific CSS (ako imaš fajl; neće menjati izgled strelica) */
...
/* 2) Meta: hero_ids + intro_text */
$meta = function_exists('wi_get_digital_meta') ? wi_get_digital_meta($page_id) : [];
...
if (empty($slides)) {
  $slides = [
    'https://picsum.photos/id/1100/1600/900',
    'https://picsum.photos/id/1101/1600/900',
    'https://picsum.photos/id/1102/1600/900',
  ];
}
...
/* 5) Intro tekst iz metabox-a (fallback ako je prazno) */
$intro_text = !empty($meta['intro_text'])
  ? $meta['intro_text']
  : 'Digital Signage verbindet Content, Technik und Reichweite: von Shop- und Messelösungen über Schaufenster bis zu Großformaten. WERBEINSEL plant, gestaltet und betreibt Ihre digitalen Flächen inklusive CMS, Content-Erstellung, Monitoring und Wartung.';
?>
...
<section id="<?php echo esc_attr($carousel_id); ?>" class="svc-hero-carousel">
  <div class="svc-hero-carousel__wrap">
    ...
    <!-- VIEWPORT: u njega stavljamo i NAV i DOTS (da budu centrirani po visini) -->
    <div class="svc-hero-carousel__viewport">
      ...
      <!-- Strelice (izgled ostaje identičan — koristimo postojeće klase) -->
...
<!-- INTRO: kompletan tekst iz metabox-a, sačuvaj nove redove -->
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  // Preflight validacije slika → nakon toga kreiramo dots
