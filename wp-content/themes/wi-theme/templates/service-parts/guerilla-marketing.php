<?php
/**
 * Partial: Guerilla Marketing – hero (naslov + slider) + intro tekst (FORCIRAN)
 * Vidi promenljive iz parent template-a: $slug, $svc, $page_title, $page_id
 */

/* 1) Poseban CSS samo za ovu stranicu */
$css_path = get_stylesheet_directory() . '/css/service-guerilla-marketing.css';
$css_url  = get_stylesheet_directory_uri() . '/css/service-guerilla-marketing.css';
...
if (function_exists('wi_get_guerilla_meta')) {
  $meta = wi_get_guerilla_meta($page_id);
...
if (empty($slides)) {
  // fallback – zameni svojim fotkama po želji
  $slides = [
...
$carousel_id = ($slug ?: 'guerilla-marketing') . '-hero';

/* 4) NASLOV – uvek forsiran */
$intro_title = "GUERILLA\nMARKETING";

/* 5) INTRO TEKST – uvek forsiran (ignorišemo meta['intro_text']) */
$intro_text = 'Guerilla Marketing schafft überraschende, aufmerksamkeitsstarke Erlebnisse im öffentlichen Raum. Mit kreativen, unkonventionellen Aktionen erreichen Sie Menschen dort, wo sie es am wenigsten erwarten – viral, teilbar und budgeteffizient. WERBEINSEL entwickelt, produziert und realisiert Guerilla-Kampagnen inklusive Genehmigungen, Sicherheitskonzept und messbaren Zielen.';
?>
...
<section id="<?php echo esc_attr($carousel_id); ?>" class="svc-hero-carousel">
  <div class="svc-hero-carousel__wrap">
    ...
    <!-- viewport spušten na z-index:1 i “neklikal” da ne preklapa intro -->
    <div class="svc-hero-carousel__viewport" style="z-index:1; pointer-events:none;">
      <?php foreach ($slides as $i => $url): ?>
        <div
          ...
          role="img"
          aria-label="Guerilla Marketing Slide <?php echo (int)$i+1; ?>">
        </div>
      <?php endforeach; ?>
    </div>

    <!-- Navigacija preko viewporta (klikalna) -->
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  // Anti-clamp guard (ako neka skripta posle pokuša da skrati)
