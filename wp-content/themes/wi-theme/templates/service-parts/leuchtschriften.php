<?php
/**
 * Partial: Leuchtschriften – hero (naslov + slider) + intro (editabilno)
 * Vidi: $slug, $svc, $page_title, $page_id
 */

/* Page-specific CSS (opciono; ne menja izgled strelica) */
$css_path = get_stylesheet_directory() . '/css/service-leuchtschriften.css';
$css_url  = get_stylesheet_directory_uri() . '/css/service-leuchtschriften.css';
...
$meta = function_exists('wi_get_leucht_meta') ? wi_get_leucht_meta($page_id) : [];
...
if (empty($slides)) {
  ...
  $slides = [
    'https://picsum.photos/id/2100/1600/900',
    'https://picsum.photos/id/2101/1600/900',
    'https://picsum.photos/id/2102/1600/900',
  ];
}
if (!empty($svc['image_url'])) { $slides[] = $svc['image_url']; }
$slides = array_values(array_unique(array_filter($slides)));

$carousel_id = ($slug ?: 'leuchtschriften') . '-hero';

/* Naslov (forsiran) — jedna reč */
$intro_title = "LEUCHTSCHRIFTEN";

/* Intro tekst iz metabox-a + fallback (bez skraćivanja) */
$intro_text = !empty($meta['intro_text'])
  ? $meta['intro_text']
  : 'Leuchtschriften verbinden Markenwirkung mit Sichtbarkeit: Einzelbuchstaben, Profilbuchstaben, Kantenleuchter, Rückleuchter, 3D-Logos i specijalne izvedbe. WERBEINSEL preuzima dizajn, proizvodnju, montažu i servis – prilagođeno vašoj lokaciji.';
?>
...
<section id="<?php echo esc_attr($carousel_id); ?>" class="svc-hero-carousel">
  <div class="svc-hero-carousel__wrap">
    ...
    <!-- VIEWPORT: nav + dots unutra, strelice centrirane po visini -->
    <div class="svc-hero-carousel__viewport">
      <?php foreach ($slides as $i => $url): ?>
        <div
          ...
          role="img"
          aria-label="Leuchtschriften Slide <?php echo (int)$i+1; ?>">
        </div>
      <?php endforeach; ?>
