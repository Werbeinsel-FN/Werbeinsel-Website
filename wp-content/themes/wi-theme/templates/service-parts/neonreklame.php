<?php
/**
 * Partial: Neonreklame – hero (naslov + slider) + intro tekst (FORCIRAN)
 * Vidi promenljive iz parent template-a: $slug, $svc, $page_title, $page_id
 */

/* 1) Poseban CSS samo za ovu stranicu */
$css_path = get_stylesheet_directory() . '/css/service-neonreklame.css';
$css_url  = get_stylesheet_directory_uri() . '/css/service-neonreklame.css';
...
if (empty($slides)) {
  // fallback – zameni realnim vizualima
  $slides = [
    'https://picsum.photos/id/133/1600/900',
    'https://picsum.photos/id/134/1600/900',
    'https://picsum.photos/id/135/1600/900',
  ];
}
if (!empty($svc['image_url'])) { $slides[] = $svc['image_url']; }
$slides = array_values(array_unique(array_filter($slides)));

$carousel_id = ($slug ?: 'neonreklame') . '-hero';

/* 4) NASLOV – uvek forsiran */
$intro_title = "NEON\nREKLAME";

/* 5) INTRO TEKST – uvek forsiran (ignorišemo meta['intro_text']) */
$intro_text = 'Neonreklame setzen Marken ikonisch in Szene – sichtbar bei Tag und unübersehbar bei Nacht. Von klassischen Neon-Schriften über LED-Neon bis zu individuellen Leuchtlogos planen und produzieren wir langlebige Lösungen inkl. Statik, Genehmigungen, Montage und Wartung. WERBEINSEL bringt Ihr Lichtzeichen sicher und normgerecht an die Fassade – mit maximaler Strahlkraft.';
?>
    ...
    <div class="svc-hero-carousel__viewport" style="z-index:1; pointer-events:none;">
      <?php foreach ($slides as $i => $url): ?>
        <div
          ...
          role="img"
          aria-label="Neonreklame Slide <?php echo (int)$i+1; ?>">
        </div>
      <?php endforeach; ?>
    </div>

    <!-- Navigacija i tačkice iznad viewporta (klikalni) -->
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  // Anti-clamp guard (za svaki slučaj)
