<?php
/**
 * Partial: Print Design – hero (naslov + slider) + intro (editabilno)
 * Vidi: $slug, $svc, $page_title, $page_id
 */

/* Page-specific CSS (opciono; ne menja izgled strelica) */
$css_path = get_stylesheet_directory() . '/css/service-print-design.css';
$css_url  = get_stylesheet_directory_uri() . '/css/service-print-design.css';
...
$meta = function_exists('wi_get_print_meta') ? wi_get_print_meta($page_id) : [];
...
if (empty($slides)) {
  ...
  $slides = [
    'https://picsum.photos/id/1700/1600/900',
    'https://picsum.photos/id/1701/1600/900',
    'https://picsum.photos/id/1702/1600/900',
  ];
}
if (!empty($svc['image_url'])) { $slides[] = $svc['image_url']; }
$slides = array_values(array_unique(array_filter($slides)));

$carousel_id = ($slug ?: 'print-design') . '-hero';
...
$intro_text = !empty($meta['intro_text'])
  ? $meta['intro_text']
  : 'Print Design macht Marken greifbar: Flyer, Broschüren, Kataloge, Plakate, Verpackungen und mehr. WERBEINSEL gestaltet hochwertige Printmedien von der Idee über die Produktion bis zur Auslieferung – präzise, konsistent und wirkungsvoll.';
?>
...
<section id="<?php echo esc_attr($carousel_id); ?>" class="svc-hero-carousel">
  <div class="svc-hero-carousel__wrap">
    ...
    <!-- VIEWPORT: nav + dots unutra, strelice centrirane po visini (izgled ostaje isti) -->
    <div class="svc-hero-carousel__viewport">
      <?php foreach ($slides as $i => $url): ?>
        <div
          ...
          role="img"
          aria-label="Print Design Slide <?php echo (int)$i+1; ?>">
        </div>
      <?php endforeach; ?>
