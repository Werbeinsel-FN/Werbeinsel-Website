<?php
/**
 * Partial: UI/UX Design – hero (naslov + slider) + intro tekst (FORCIRAN)
 * Vidi promenljive iz parent template-a: $slug, $svc, $page_title, $page_id
 */

/* 1) Poseban CSS samo za ovu stranicu */
$css_path = get_stylesheet_directory() . '/css/service-ui-ux-design.css';
$css_url  = get_stylesheet_directory_uri() . '/css/service-ui-ux-design.css';
...
if (empty($slides)) {
  ...
  $slides = [
    'https://picsum.photos/id/110/1600/900',
    'https://picsum.photos/id/111/1600/900',
    'https://picsum.photos/id/112/1600/900',
  ];
}
if (!empty($svc['image_url'])) { $slides[] = $svc['image_url']; }
$slides = array_values(array_unique(array_filter($slides)));

$carousel_id = ($slug ?: 'ui-ux-design') . '-hero';

/* 4) NASLOV – uvek forsiran */
$intro_title = "UI/UX\nDESIGN";

/* 5) INTRO TEKST – uvek forsiran (ignorišemo meta['intro_text']) */
$intro_text = 'UI/UX Design verbindet Strategie, Ästhetik und Usability zu messbaren Erlebnissen. Mit klaren User Flows, konsistenten Komponenten und zugänglichen Interfaces steigern wir Conversion und Zufriedenheit. WERBEINSEL plant Research & Workshops, erstellt Wireframes und High-Fidelity-Designs inkl. Design Systemen – bis zur sauberen Übergabe an die Entwicklung.';
?>
    ...
    <div class="svc-hero-carousel__viewport" style="z-index:1; pointer-events:none;">
      <?php foreach ($slides as $i => $url): ?>
        <div
          ...
          role="img"
          aria-label="UI/UX Design Slide <?php echo (int)$i+1; ?>">
        </div>
      <?php endforeach; ?>
    </div>
