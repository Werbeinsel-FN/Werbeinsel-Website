<?php
/**
 * Partial: Webentwicklung – hero (naslov + slider) + intro (editabilno)
 * Vidi: $slug, $svc, $page_title, $page_id
 */

/* Page-specific CSS (opciono) */
$css_path = get_stylesheet_directory() . '/css/service-webentwicklung.css';
$css_url  = get_stylesheet_directory_uri() . '/css/service-webentwicklung.css';
...
$meta = function_exists('wi_get_web_meta') ? wi_get_web_meta($page_id) : [];
...
if (empty($slides)) {
  ...
  $slides = [
    'https://picsum.photos/id/1600/1600/900',
    'https://picsum.photos/id/1601/1600/900',
    'https://picsum.photos/id/1602/1600/900',
  ];
}
if (!empty($svc['image_url'])) { $slides[] = $svc['image_url']; }
$slides = array_values(array_unique(array_filter($slides)));

$carousel_id = ($slug ?: 'webentwicklung') . '-hero';
...
$intro_text = !empty($meta['intro_text'])
  ? $meta['intro_text']
  : 'Wir entwickeln performante Websites und Webapps: von Headless CMS und API-Integrationen bis zu Shop-Lösungen. WERBEINSEL liefert UX, Frontend, Backend, Hosting und laufende Betreuung – schnell, sicher und suchmaschinenfreundlich.';
?>
    ...
    <div class="svc-hero-carousel__viewport">
      <?php foreach ($slides as $i => $url): ?>
        <div
          ...
          role="img"
          aria-label="Webentwicklung Slide <?php echo (int)$i+1; ?>">
        </div>
      <?php endforeach; ?>
